...
func _init(name: String, territory: Territory) -> void:
	...
	self.ability = AbilityType.HIGHLANDER
	self.cost = 2
...
func attack_unit(enemy: Unit) -> bool:
	...
	# Highlanders deal double damage in mountain territories
	if self.current_territory.terrain == self.current_territory.TerrainType.MOUNTAIN:
		damage *= 2
